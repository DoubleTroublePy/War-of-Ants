...
fn main() -> Result<(), Error> {
    ...
    for i in 0..10 {
        ants.push(Ant::new(1, Coordinate::new((WIDTH/2) as i16, (HEIGHT/2) as i16), 30, 10, 0));
    }
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        if let Event::RedrawRequested(_) = event { 
            ...
            for ant in &mut ants {
                let x = ant.coordinate_get().0;
                let y = ant.coordinate_get().1;
        ...
        if input.update(&event) {
            ...
            if input.key_pressed(VirtualKeyCode::R) {
                for ant in &mut ants {
                    ant.coordinate_set(Coordinate::new((WIDTH/2) as i16, (HEIGHT/2) as i16));
                }
            }
